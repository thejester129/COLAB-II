...
function addMarkers() {
  images.forEach((image) => {
    ...
    if (isMobile()) {
      console.log("mobile");
      createMobileMarkers(image, popup);
    } else {
      createDesktopMarkers(image, popup);
    }
  });
}

function createMobileMarkers(image, popup) {
...
function createDesktopMarkers(image, popup) {
  var marker = L.marker(image.location, {
    icon: getPhotoIcon(image.sources[0]),
  })
